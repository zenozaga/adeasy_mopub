...
class AdEasyMopub {
  ...
  Future<bool> init(
      {String? adUnitID, bool testMode = false, bool adConsent = false}) async {
  ...
  Future<bool> setConsent({required bool enable}) async {
    return await methodChannel.invokeMethod(Constants.METHOD_SET_CONCENT, {
      "enable": enable,
    });
  }
  ...
  Future<bool> loadInterstitial(String unitID,
      [AdEasyListener? listener]) async {
    _listener = listener;
    var result = await methodChannel
        .invokeMethod(Constants.METHOD_LOAD_INTERSTITIAL, {"unitID": unitID});
  ...
  Future<bool> showInterstitial(String unitID,
    ...
    _listener = listener;
    var result = await methodChannel
        .invokeMethod(Constants.METHOD_SHOW_INTERSTITIAL, {"unitID": unitID});
  ...
  Future<bool> loadRewarded(String unitID, [AdEasyListener? listener]) async {
    _listener = listener;
    var result = await methodChannel
        .invokeMethod(Constants.METHOD_LOAD_REWARD, {"unitID": unitID});
  ...
  Future<bool> showRewarded(String unitID, [AdEasyListener? listener]) async {
    _listener = listener;
    var result = await methodChannel
        .invokeMethod(Constants.METHOD_SHOW_REWARD, {"unitID": unitID});
